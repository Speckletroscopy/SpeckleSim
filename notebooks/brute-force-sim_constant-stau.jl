...
$(@bind tmax NumberField(1.0:0.1:50.0;default=10))
...
md"""
Make separate instances of fields: $(@bind makeInstances CheckBox())
"""
...
# ╔═╡ 874462ef-e735-4f19-bbf9-ee2d2401eb15
md"""
Set seed $\rightarrow$ $(@bind setSeed CheckBox())
"""

# ╔═╡ 095ebc52-38d8-46d7-a5af-d2de0719ea92
if setSeed
	md"""
	$(@bind seed NumberField(1:1000000;default=42))
	"""
end
...
if sim	
	...
	γCorrTimes = Vector{typeof(allCorrTimes)}[]
...
if sim
	...
	xlabel!(γCountsPlot,"time (ns)")
	title!(γCountsPlot,"Total photon counts = $(sum(totCounts))")
...
if sim
	...
	title!(g2τFFTplot,"\$\\hat{g}^{(2)}(\\nu)\$")
	xlabel!(g2τFFTplot,"frequency (GHz)")
	vline!(g2τFFTplot,[shift],label="Frequency shift",ls=:dash)
end

# ╔═╡ a2d886d7-f030-4d38-860b-399e501b24fc
if sim && makeInstances
	g2τFFTsumPlot = plot(allfreqsPos,abs.(g2τ1FFTsumPos),label=false)
	title!(g2τFFTsumPlot,"\$\\sum_{i=1}^{$(convert(Integer,ceil(ntot/nbar)))}\\hat{g}_i^{(2)}(\\nu)\$")
	xlabel!(g2τFFTsumPlot,"frequency (GHz)")
	vline!(g2τFFTsumPlot,[shift],label="Frequency shift",ls=:dash)
